$(function () {
    ...
    function GetData(cb) {     
        ...
        $.ajax({  
            ...
            data:"time="+time+"&value="+value+"&host="+$('#hostIp').attr("value")+"&targetValue="+$('#targetValue').attr("value")+"&collectTime="+$("#collectTime").val(),  
            dataType:"json",  
    ...
    function cb(name,color){
        ...
        $.plot($("#ShowTimeOutUpdating"), dataset, options);
    ...
    $("#ShowTimeOutUpdating").bind("plothover", function (event, pos, item) {
        $("#x").text(pos.x.toFixed(2));
        $("#y").text(pos.y.toFixed(2));

        if ($("#ShowTimeOutUpdating").length > 0) {
            if (item) {
